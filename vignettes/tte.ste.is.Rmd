...
## Load the package
If you haven't already installed it, please see instructions at the bottom of [my repository](https://github.com/annam21/TTE.STE.IS)
```{r, message = F}
library(TTE.STE.IS)
```
...
As shown above, the database must contain the following columns (at a minimum, and in any order):
 - cam (name must be exact). This is the unique ID of the sampling site. This column can be any class of object
 - datetime (name must be exact). This is the date and time of the photo, in class POSIXct
 - a (name must be exact). This is the viewable area of that camera at that date and time. Class numeric. This column is optional if you create your camera area database through different means than a_lookup_fn (described below)
...
## Space-to-Event
### Encounter history
To create your STE encounter history, you must define the first and last days of your study period, and then use the functions ste_data_fn (see its documentation for more details).
```{r}
